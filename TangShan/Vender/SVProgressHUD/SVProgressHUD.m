...
+ (void)showStatusWithoutPic:(NSString *)string duration:(NSTimeInterval)duration{
    [SVProgressHUD show];
    [SVProgressHUD dismissWithoutPic:string afterDelay:duration];
}
...
+ (void)dismissWithoutPic:(NSString *)errorString afterDelay:(NSTimeInterval)seconds {
    [[SVProgressHUD sharedView] dismissWithStatusWithoutPic:errorString error:YES afterDelay:seconds];
}
...
- (void)setStatusWithouPic:(NSString *)string {
	...
	self.stringLabel.hidden = NO;
	self.stringLabel.text = string;
	self.stringLabel.frame = CGRectMake(10, (hudHeight-stringHeight)/2, hudWidth-20, stringHeight);
	
    self.spinnerView.center =  CGPointMake(CGRectGetWidth(self.hudView.bounds)/2, 36);
...
- (void)dismissWithStatusWithoutPic:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
	...
	[self setStatusWithouPic:string];
