...
- (void)loadViewFrame
{
    ...
    UILabel* locationLable=[[UILabel alloc]initWithFrame:CGRectMake(0, 22, WIDETH, 44)];
    locationLable.textColor=[UIColor whiteColor];
    locationLable.textAlignment=NSTextAlignmentCenter;
    locationLable.font=[UIFont boldSystemFontOfSize:18];
    locationLable.text=@"意见反馈";
    [self.view addSubview:locationLable];
    
    
    UIButton* ADAF=[[UIButton alloc]initWithFrame:CGRectMake(WIDETH-80, 22, 80, 44)];
    ADAF.backgroundColor=[UIColor clearColor];
    [ADAF setTitle:@"关闭" forState:UIControlStateNormal];
    [ADAF setTitle:@"关闭" forState:UIControlStateHighlighted];
    [ADAF setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [ADAF setTitleColor:[UIColor whiteColor] forState:UIControlStateHighlighted];
    [self.view addSubview:ADAF];
    
    [ADAF addTarget:self action:@selector(ADAFDAFSFSA) forControlEvents:UIControlEventTouchUpInside];
...
- (void)loadView {
    ...
    UIImageView* adafsa=[[UIImageView alloc]initWithFrame:CGRectMake(0, 0, WIDETH, HEIGHTH-44)];
    adafsa.image=IMAGE(@"dsafs");
    [self.view addSubview:adafsa];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (content.length > 0) {
        CGSize labelSize = [content boundingRectWithSize:CGSizeMake(self.mTableView.frame.size.width - 40, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14.0f]} context:nil].size;
