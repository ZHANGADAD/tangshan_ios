...
@property(assign,nonatomic)NSInteger chooseColor;

@property(assign,nonatomic)NSInteger indexChooseItem;

@end

@implementation UUChart
@synthesize count;
@synthesize commonDataSource;

-(id)initwithUUChartDataFrame:(CGRect)rect withSource:(id<UUChartDataSource>)dataSource withStyle:(UUChartStyle)style withColor:(NSInteger)chooseColor withIndexChoose:(NSInteger)indexChooseItem{
    self.dataSource = dataSource;
    self.chartStyle = style;
    self.chooseColor=chooseColor;
    self.indexChooseItem=indexChooseItem;
...
-(void)setUpChart{
	if (self.chartStyle == UUChartStyleLine) {
        if(!_lineChart){
            ...
            _lineChart.chooseColor=self.chooseColor;
        ...
        _lineChart.indexChooseItem=self.indexChooseItem;
        _lineChart.commonDataSource=self.commonDataSource;
        //判断显示最大最小值
